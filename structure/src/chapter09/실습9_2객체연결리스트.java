...
class LinkedList2 {
	...
	// 전달된 element를 찾을 때 comparator 객체를 사용한다
	{
		...
		Node2 p = first;
		while (p != null) {
			if (cc.compare(p.data, element) == 0) {// 삭제할 데이터가 존재할 때
				if (q == null) {// 기존 데이터가 한개인 경우
					first = p.link;
				} else {// 기존 데이터가 여러개인 경우
					q.link = p.link;
				}
				return p.data.NO;// 데이터의 번호를 반환한다?
				
			} else {// 두개의 데이터가 같지 않을 때
				
				q = p;
				p = p.link;//삭제할 대상을 찾아서 움직인다.
			}
		}
		return -1;// 삭제할 대상이 없다.
	}
	...
	// 임의 값을 삽입할 때 리스트가 오름차순으로 정렬이 되도록 한다
	{
		...
		Node2 q = null, p = first;

		if (first == null) // first에 데이터가 없을때(데이터가 처음 삽입될 때)
		{
			first = newNode;
			return;
		}
		while (p != null) {// 데이터가 존재하는 경우
			if (cc.compare(p.data, element) < 0) {// 기존데이터보다 새로운데이터가 클 때
				q = p;
				p = p.link;
			} else {// 기존데이터보다 새로운데이터가 작거나 같을 때
				// 새데이터가 제일 앞에 들어가는 경우
				if (q == null) {
					newNode.link = p;
					first = newNode;
		...
		} // while문을 p!=null로 설정했으므로 마지막노드일 경우를 제외 했으므로
		//마지막에 노드를 넣는경우를 적어준다.
		q.link = newNode;
		...
		do {
			...
			case Merge:// 난이도 상
				for (int i = 0; i < count; i++) {// 3개의 객체를 연속으로 입력받아 l2 객체를 만든다
					data = new SimpleObject5();
					data.scanData("병합", 3);
					l2.Add(data, SimpleObject5.NO_ORDER);// 새로운 리스트를 만들어서 합친다.
				}
				l.Merge(l2);// 옵션->어려운것
				break;
			case Exit: // 꼬리 노드 삭제
